...
class TitleDetail extends AbstractExtractionRule implements ExtractionRule
{
    public function proceed(\SimpleXMLElement $element): void
    {
        if (isset($element->TitleDetail)) {
            foreach ($element->TitleDetail as $title) {
                $titleDetail = new \Adimeo\Onix\Entity\Product\TitleDetail();
                ...
                $this->product->getDescriptiveDetail()->addTitleDetail($titleDetail);
            }
        }
    }
}
